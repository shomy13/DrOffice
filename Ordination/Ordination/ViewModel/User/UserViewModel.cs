...
namespace Ordination.ViewModel.User
{
    class UserViewModel : TabViewModel
    {
        ...
        void PasswordChange()
        {
         UserChangePaswordViewModel tab = this.ContentTab.FirstOrDefault(vm => vm is UserChangePaswordViewModel)
                as UserChangePaswordViewModel;

            if (tab == null)
            {
                tab = new UserChangePaswordViewModel();
                this.ContentTab.Add(tab);
                this.SetActiveTab(tab);
            }
        }
